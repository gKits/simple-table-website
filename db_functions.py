    ...
    try:
        with get_connection(database) as conn:
            with open(path_to_script, 'r') as f:
                result = conn.executescript(f.read())
...
def insert_into_table(app, database: str, table: str, kwargs):
    try:
        print(kwargs)
        with get_connection(database) as conn:
            k_string = ', '.join(kwargs.keys())
            v_string = ', '.join(kwargs.values())
            conn.execute(
                f'INSERT INTO {table} ({k_string})\nVALUES({v_string});'
            )
        app.logger.info(f'{kwargs} was inserted into "{database}/{table}"')
...
def type_casting(app, type: str, value):
    TYPE_CASTING = {
        'TEXT': str,
        'INTEGER': int,
        'REAL': float
